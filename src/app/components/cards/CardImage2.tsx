...
export default function ImageCard2({ imageUrl, title }: ImageCardProps) {
  return (
    <div
      className="flex-1 min-w-[196px] max-w-[240px] overflow-hidden rounded-2xl shadow-lg transition-transform duration-300 hover:scale-105"
      role="group"
      aria-label={`Imagem de ${title} com link para WhatsApp`}
    >
      <figure className="relative w-full h-[360px]">
        <Image
          src={imageUrl}
          alt={title}
          fill
